...
# take the summary and branch out the chain in two different arms
# one will return the summary as it is
# the other will return the length of the summary

parallel_runnable = RunnableParallel(
    summary = lambda x: x, # to return summary as it is
    length = lambda x: len(x) # to return the length of the summary
)

parallel_chain = summarize_chain | parallel_runnable

output = parallel_chain.invoke({"context": "what is langchain?"})

# the output is a dictionary containing the summary and the length of the summary
